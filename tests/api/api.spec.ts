import Endpoints from '../../src/api/enums/enpoints.enum';
import HttpStatusCode from '../../src/api/enums/HttpStatusCode.enum';
import { test, expect } from './../../src/api/fixtures/API.fixture';
import Ajv from 'ajv';
import addFormats from 'ajv-formats';
import baseAPI from '../../src/api/utils/baseAPI';
import { createNewPost } from '../../src/api/test-data/createNewPost.fixture';
...
test.describe('API tests for jsonPlaceholer', async () => {
  ...
  test('Should check GET posts schema', async ({ API }) => {
    ...
    expect(response.status()).toBe(HttpStatusCode.OK);
    responseJson = await response.json();
    valid = ajv.validate(require('../../src/api/test-data/posts.schema.json'), responseJson);
    if (!valid) {
  ...
  test('Should check GET post schema', async ({ API }) => {
    ...
    responseJson = await response.json();
    valid = ajv.validate(require('../../src/api/test-data/post.schema.json'), responseJson);
  ...
  test('Should check GET comments schema', async ({ API }) => {
    ...
    responseJson = await response.json();
    valid = ajv.validate(require('../../src/api/test-data/comments.schema.json'), responseJson);
